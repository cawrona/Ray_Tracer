# Name: Christian Wrona
# Email: cwrona2@uwo.ca
# Description: A ray-tracing program that displays spheres of various colors, and computes shadows generated by objects.

import operator
from math import *
import numpy as np
from matrix import matrix

class cameraMatrix:

    def __init__(self,window,UP,E,G,nearPlane=10.0,farPlane=50.0,theta=90.0):
        self.__UP = UP.normalize()
        self.__E = E
        self.__G = G
        self.__np = nearPlane
        self.__fp = farPlane
        self.__width = window.getWidth()
        self.__height = window.getHeight()
        self.__theta = theta
        self.__aspect = self.__width/self.__height
        self.__npHeight = self.__np*(pi/180.0*self.__theta/2.0)
        self.__npWidth = self.__npHeight*self.__aspect

        Mp = self.__setMp(self.__np,farPlane)
        T1 = self.__setT1(self.__np,self.__theta,self.__aspect)
        S1 = self.__setS1(self.__np,self.__theta,self.__aspect)
        T2 = self.__setT2()
        S2 = self.__setS2(self.__width,self.__height)
        W2 = self.__setW2(self.__height)

        self.__N = (self.__E - self.__G).removeRow(3).normalize()
        self.__U = self.__UP.removeRow(3).crossProduct(self.__N).normalize()
        self.__V = self.__N.crossProduct(self.__U)

        self.__Mv = self.__setMv(self.__U,self.__V,self.__N,self.__E)
        self.__C = W2*S2*T2*S1*T1*Mp
        self.__M = self.__C*self.__Mv

    def __setMv(self,U,V,N,E):
        Mv = matrix(np.identity(4))
        Mv.set(0,0,U.get(0,0))
        Mv.set(0,1,U.get(1,0))
        Mv.set(0,2,U.get(2,0))
        Mv.set(0,3,-E.removeRow(3).dotProduct(U))

        Mv.set(1,0,V.get(0,0))
        Mv.set(1,1,V.get(1,0))
        Mv.set(1,2,V.get(2,0))
        Mv.set(1,3,-E.removeRow(3).dotProduct(V))

        Mv.set(2,0,N.get(0,0))
        Mv.set(2,1,N.get(1,0))
        Mv.set(2,2,N.get(2,0))
        Mv.set(2,3,-E.removeRow(3).dotProduct(N))
        return Mv

    def __setMp(self,nearPlane,farPlane):
        Mp = matrix(np.identity(4))
        Mp.set(0,0,nearPlane)
        Mp.set(1,1,nearPlane)
        Mp.set(2,2,-(farPlane+nearPlane)/(farPlane-nearPlane))
        Mp.set(2,3,-2.0*(farPlane*nearPlane)/(farPlane-nearPlane))
        Mp.set(3,2,-1.0)
        Mp.set(3,3,0.0)
        return Mp

    def __setT1(self,nearPlane,theta,aspect):
        top = nearPlane*tan(pi/180.0*theta/2.0)
        right = aspect*top
        bottom = -top
        left = -right
        T1 = matrix(np.identity(4))
        T1.set(0,3,-(right+left)/2.0)
        T1.set(1,3,-(top+bottom)/2.0)
        return T1

    def __setS1(self,nearPlane,theta,aspect):
        top = nearPlane*tan(pi/180.0*theta/2.0)
        right = aspect*top
        bottom = -top
        left = -right
        S1 = matrix(np.identity(4))
        S1.set(0,0,2.0/(right-left))
        S1.set(1,1,2.0/(top-bottom))
        return S1

    def __setT2(self):
        T2 = matrix(np.identity(4))
        T2.set(0,3,1.0)
        T2.set(1,3,1.0)
        return T2

    def __setS2(self,width,height):
        S2 = matrix(np.identity(4))
        S2.set(0,0,width/2.0)
        S2.set(1,1,height/2.0)
        return S2

    def __setW2(self,height):
        W2 = matrix(np.identity(4))
        W2.set(1,1,-1.0)
        W2.set(1,3,height)
        return W2

    def getRay(self,window,i,j):
        a = -self.__np
        b = self.__npWidth*(2.0*i/window.getWidth() - 1.0)
        c = self.__npHeight*(2.0*(window.getHeight() - (j+1))/window.getHeight() - 1.0)
        return (self.__N.scalarMultiply(a) + self.__U.scalarMultiply(b) + self.__V.scalarMultiply(c)).insertRow(3,0.0)


    """"
    minimumIntersection(direction, objectList)
    ------------------------------------------
    For a given ray, finds the t-value of the intersection the the ray makes with the object in the scene.

    Parameters
    ----------
    direction: 
        vector d describing the ray r(t) = e + dt
    objectList:
        list of objects in the scene
        
    Returns
    -------
    The minimum t-value, as well as the position of the object in objectList
    """

    def minimumIntersection(self,direction,objectList):
        intersectionList = []

        #Complete this method

        #My Code
        for k in range(len(objectList)):

            Minv = objectList[k].getTinv()      # object[k] is transformed back into generic form for efficient computation

            Te = Minv*self.__E                  # Components e and d of r(t) are trasformed into generic space as well
            Td = Minv*direction
            tnot = objectList[k].intersection(Te, Td)   # Compute intersection

            if (tnot != -1.0):                          # tnot != -1.0 IS and intersection, so add to intersectionList
                intersectionList.append((k, tnot))

        intersectionList.sort(key = lambda x: x[1])     # sort intersectionList using python's built in sort()
                                                        # key = lambda x: x[1] sorts by the second element which is the intersection
                                                        # this sorts the list in order of increasing t-values (ascending by default)
        return intersectionList

    def worldToViewingCoordinates(self,P):
        return self.__Mv*P

    def worldToImageCoordinates(self,P):
        return self.__M*P

    def worldToPixelCoordinates(self,P):
        return self.__M*P.scalarMultiply(1.0/(self.__M*P).get(3,0))

    def viewingToImageCoordinates(self,P):
        return self.__C*P

    def viewingToPixelCoordinates(self,P):
        return self.__C*P.scalarMultiply(1.0/(self.__C*P).get(3,0))

    def imageToPixelCoordinates(self,P):
        return P.scalarMultiply(1.0/P.get(3,0))

    def getUP(self):
        return self.__UP

    def getU(self):
        return self.__U

    def getV(self):
        return self.__V

    def getN(self):
        return self.__N

    def getE(self):
        return self.__E

    def getG(self):
        return self.__G

    def getMv(self):
        return self.__Mv

    def getC(self):
        return self.__C

    def getM(self):
        return self.__M

    def getNp(self):
        return self.__np

    def getFp(self):
        return self.__fp

    def getTheta(self):
        return self.__theta

    def getAspect(self):
        return self.__aspect

    def getWidth(self):
        return self.__width

    def getHeight(self):
        return self.__height

    def getNpHeight(self):
        return self.__npHeight

    def getNpWidth(self):
        return self.__npWidth


class shader:

    """"
    __shadowed(object, I, S, objectList)
    -----------
    Determines if a pixel is in the shadow of another object, or of itself, by forming a ray from the intersection point to the light source.
    If the ray intersects an object, then the pixel is shadowed.

    Parameters
    ----------
    object:
        object that is being shaded
    I:
        the intersection point on that object
    S:
        vector to the lightsource
    objectList:
        list of objects in the scene
        
    Returns
    -------
    True if the ray from the intersection point to the light source intersects an object, False otherwise.
    """

    def __shadowed(self,object,I,S,objectList):
        
    #Complete this helper method
        M = object.getT()
        E = 0.001
        I = M*(I+S.scalarMultiply(E))
        S = M*S
        
        for shadowObject in objectList:
            Minv = shadowObject.getTinv()
            Iinv = Minv*I
            Sinv = Minv*S

            Sinv.normalize()

            if shadowObject.intersection(Iinv,Sinv) != -1.0:
                return True

        return False

    """"
    Constructor
    -----------
    For a given ray and its corresponding pixel (i,j) in the image, the constructor computes the shaded color for the pixel (i,j).

    Parameters
    ----------
    intersection:
        the first tuple (k, tnot) from the intersection list
    direction: 
        vector d describing the ray r(t) = e + dt
    camera:
        cameraMatrix object
    objectList:
        list of objects in the scene
    light:
        lightsource object
    """
    def __init__(self,intersection,direction,camera,objectList,light):

        
        self.__color = (0,0,0)


        k = intersection[0]     
        tnot = intersection[1]
        object = objectList[k]
        Minv = object.getTinv()

        Ts = Minv*light.getPosition()   # Transform light position and r(t) components into generic space
        Te = Minv*camera.getE()
        Td = Minv*direction

        I = Te+(Td.scalarMultiply(tnot))    # Compute intersection point

        S = (Ts.removeRow(3) - I.removeRow(3)).normalize().insertRow(3, 0.0)      # Vector from I to light source position  
        N = object.normalVector(I)                                                # Normal to the intersection point
        R = S.scalarMultiply(-1.0) + N.scalarMultiply(2.0*(S.dotProduct(N)))      # Specular Reflection Vector
        V = (Te.removeRow(3)-I.removeRow(3)).normalize().insertRow(3, 0.0)        # Vector to center of the projection (Te-I)

        # Compute local elements of shading
        Id = (N.transpose()*S).get(0,0)                 # Compute intensity of diffuse and specular light
        Is = (R.transpose()*V).get(0,0)                 
        if Id < 0.0: Id = 0.0
        if Is < 0.0: Is = 0.0
        r = object.getReflectance()                     # Retrieve reflectance coefficients
        c = object.getColor()                           # Retrieve RGB Intensities
        Li = light.getIntensity()

        if self.__shadowed(object, I, S, objectList):       # If the positioin of the object is in the shadow of another object, compute
            f = r[0]                                        #  only the ambient light. 

        else:
            f = r[0] + r[1]*Id + r[2]*Is**r[3]

        self.__color = ((int(c[0]*Li[0]*f), int(c[1]*Li[1]*f), int(c[2]*Li[2]*f)))  # Compute color of the pixel

        

    def getShade(self):
        return self.__color